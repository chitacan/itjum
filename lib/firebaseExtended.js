...
Ref.prototype.paginate$ = function() {
  const paginate = new firebase.util.Paginate(this, ...arguments);
  const fromEvent = Rx.Observable.fromEvent;
  paginate.updated$ = Rx.Observable.merge(
      fromEvent(paginate, 'child_added'),
      fromEvent(paginate, 'child_changed')
    );
  paginate.removed$ = fromEvent(paginate, 'child_removed');
  return paginate;
};
